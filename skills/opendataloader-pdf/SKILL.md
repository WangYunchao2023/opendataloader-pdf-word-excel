...
description: 文档解析工具（统一处理 PDF / Word），自动检测文档类型、智能选择模式（Fast/Hybrid）。Word 输入时采用「docx内容 + PDF位置」双来源智能合并架构——表格/文本内容以 docx 为准，页码/section_path 以 PDF 为准，输出含完整内容和位置追溯信息。
---
...
基于 [OpenDataLoader PDF](https://github.com/opendataloader-project/opendataloader-pdf) 的文档解析工具，支持 **PDF 和 Word 统一处理**。**核心特性：自动检测文档类型并智能选择处理模式。**
